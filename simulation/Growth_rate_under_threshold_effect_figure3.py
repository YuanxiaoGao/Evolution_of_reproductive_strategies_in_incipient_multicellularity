...
b_cluster=int(sys.argv[1])              # from 0 to grid_num     grid_num points
m_cluster=int(sys.argv[2])              # from 0 to grid_num      grid_num points
k_cluster=int(sys.argv[3])		                # two value +-1 to make the two figures
i_th=int(sys.argv[4])                   # i_th lc in lcc     len(lc_data)
...
grid_m=11                                        # 13[5], 23[6], 37[7]; 58[8]; 87[9]; 128[10]

b_range=np.array([1,16])
m_range=np.array([0,0.1])
k_range=np.array([1,2,3,4,5,6,7,8])     # first take 1,2

grid_b=np.linspace(b_range[0],b_range[1],num=grid_num,endpoint=True)
grid_m=np.linspace(m_range[0],m_range[1],num=grid_m,endpoint=True)
...
with open('data/%d_%d_%d_%d.txt'%(b_cluster,m_cluster,k_cluster,i_th), 'w') as f:

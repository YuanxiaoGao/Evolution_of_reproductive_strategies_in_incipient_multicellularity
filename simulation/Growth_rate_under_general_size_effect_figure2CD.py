...
#----ramdom time list -------------
num_lines=2000
with open('./size_tsn_line%s_v0.txt'%num_lines, 'rb') as fp:
	t_data0 = pickle.load(fp)
	
test_number=1000                       # test the results under 1000 lines
t_data=t_data0[0:test_number]	
#------------------------------------------------------------------------------------------------------------
'''exhaustive cluster parameters'''

T_cluster=int(sys.argv[1])              # from 0 to test_number
i_th=int(sys.argv[2])                   # i_th lc in lcc     len(lc_data)

'''transform cluster parameters into local parameters'''
t_time_list=t_data[T_cluster]
t_neutral=np.array([np.log((item+1)/item) for item in range(1,8)])
t_time_list1=np.array(t_time_list)*t_neutral
...
def T(C):
	num_cell=(C[0]+C[1])
	coef= t_time_list1[num_cell-1]                                          # netural coefficient ln[i+j+1]/[i+j]
...
with open('data/%d_%d.txt'%(T_cluster,i_th), 'w') as f:
